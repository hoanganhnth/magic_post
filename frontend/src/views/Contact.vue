<!-- App.vue -->

<template>
  <div>
    <page-header></page-header>
    <div class="image-container">
      <h1>Liên hệ</h1>
    </div>
    <div class="address">
      <div class="address-contact-item">
        <div class="img-top-contact">
          <img
            src="data:image/png;base64,"
            alt="icon company"
          />
        </div>
        <div class="company-name-contact">Trụ sở chính</div>
        <ul class="list-address">
          <li class="item-address">
            Tầng 4 , Toà nhà FLC Landmark Tower, ngõ 5 Lê Đức Thọ, Mỹ Đình 2,
            Nam Từ Liêm, Hà Nội
          </li>
          <li class="item-address">
            Điện thoại: <a href="tel:1900545433">1900545433</a>
          </li>
          <li class="item-address">
            Email: <a href="mailto:cskh@ems.com.vn">cskh@ems.com.vn</a>
          </li>
        </ul>
      </div>
      <div class="address-contact-item">
        <div class="img-top-contact">
          <img
            src="data:image/png;base64,"
            alt="icon company"
          />
        </div>
        <div class="company-name-contact">Chi nhánh Hà Nội</div>
        <ul class="list-address">
          <li class="item-address">
            Tầng 4 , Toà nhà FLC Landmark Tower, ngõ 5 Lê Đức Thọ, Mỹ Đình 2,
            Nam Từ Liêm, Hà Nội
          </li>
          <li class="item-address">
            Điện thoại: <a href="tel:1900545433">1900545433</a>
          </li>
          <li class="item-address">
            Email: <a href="mailto:cskh@ems.com.vn">cskh@ems.com.vn</a>
          </li>
        </ul>
      </div>
      <div class="address-contact-item">
        <div class="img-top-contact">
          <img
            src="data:image/png;base64,"
            alt="icon company"
          />
        </div>
        <div class="company-name-contact">Chi nhánh Đà Nẵng</div>
        <ul class="list-address">
          <li class="item-address">
            Tầng 4 , Toà nhà FLC Landmark Tower, ngõ 5 Lê Đức Thọ, Mỹ Đình 2,
            Nam Từ Liêm, Hà Nội
          </li>
          <li class="item-address">
            Điện thoại: <a href="tel:1900545433">1900545433</a>
          </li>
          <li class="item-address">
            Email: <a href="mailto:cskh@ems.com.vn">cskh@ems.com.vn</a>
          </li>
        </ul>
      </div>
      <div class="address-contact-item">
        <div class="img-top-contact">
          <img
            src="data:image/png;base64,"
            alt="icon company"
          />
        </div>
        <div class="company-name-contact">Chi nhánh Hồ Chí Minh</div>
        <ul class="list-address">
          <li class="item-address">
            Tầng 4 , Toà nhà FLC Landmark Tower, ngõ 5 Lê Đức Thọ, Mỹ Đình 2,
            Nam Từ Liêm, Hà Nội
          </li>
          <li class="item-address">
            Điện thoại: <a href="tel:1900545433">1900545433</a>
          </li>
          <li class="item-address">
            Email: <a href="mailto:cskh@ems.com.vn">cskh@ems.com.vn</a>
          </li>
        </ul>
      </div>
    </div>
    <div class="row">
      <div class="col-lg-6 col-12">
        <div class="contact__left-form">
          <v-img
            class="contact__left-form-img"
            src="../images/img-contact1-effb3eda225cc89caec0a1281d616ddd.png"
            alt="img contact"
          ></v-img>
          <div class="contact__left-form-title">
            Hỗ trợ trực tuyến 24/7 thông tin giá cước và dịch vụ
          </div>
        </div>
      </div>
      <div class="col-lg-6 col-12">
        <div class="contact__right-form">
          <div class="contact__right-form-title">
            <img
              class="contact__right-form-title-icon"
              src="data:image/png;base64,"
              alt="icon email"
            />
            <div class="contact__right-form-title-title">
              Liên hệ với chúng tôi
            </div>
          </div>
          <form class="form-contact" siq_id="autopick_2474">
            <div class="form-group">
              <input
                type="text"
                required=""
                placeholder="Tên của bạn"
                name="name"
                value=""
                autocomplete="off"
                class="input-custom form-control form-control-lg"
              />
            </div>
            <div class="form-group">
              <input
                type="email"
                required=""
                placeholder="Email của bạn"
                name="email"
                value=""
                autocomplete="off"
                class="input-custom form-control form-control-lg"
              />
            </div>
            <div class="form-group">
              <textarea
                required=""
                placeholder="Tin nhắn"
                name="content"
                autocomplete="off"
                class="input-custom area form-control form-control-lg"
              ></textarea>
            </div>
            <button type="submit">Gửi yêu cầu</button>
          </form>
        </div>
      </div>
    </div>
    <page-footer></page-footer>
  </div>
</template>

<script>
import PageFooter from "@/components/PageFooter.vue";
import PageHeader from "@/components/PageHeader.vue";

// import PageHeader from '@/components/PageHeader.vue';

export default {
  name: "Contact",
  components: {
    PageHeader,
    PageFooter,

    //  PageHeader,
  },
};
</script>

<style scoped>
.image-container {
  color: white;
  height: 400px;
  background: url(../images/contact1.jpg);
  background-position: top;
  background-repeat: no-repeat;
  background-size: cover;
  padding: 2rem 1rem;
  margin-bottom: 2rem;
}

.address {
  display: flex;
  justify-content: space-around;
  align-items: center;
  margin-left: 10%;
  margin-right: 10%;
  /* Để các phần tử trên dòng mới khi không đủ không gian */
}

.company-name-contact {
  font-style: normal;
  font-weight: 700;
  font-size: 22px;
  line-height: 27px;
  color: #0072bc;
  margin-bottom: 20px;
}

.img-top-contact img {
  display: block;
  width: 50px; /* Đặt chiều rộng mong muốn */
  height: auto;

  border-style: none;
  overflow-clip-margin: content-box;
  overflow: clip;
}

li {
  display: list-item;
  text-align: -webkit-match-parent;
}

ul {
  margin: 5%;
  padding: 0;
  list-style: none;
}
.contact__left-form .contact__left-form-img {
  margin: 10%;
  margin-bottom: 20px;
  border-radius: 10px;
  vertical-align: middle;
  border-style: none;
  width: 80%;
}
.row {
  background-color: antiquewhite;
}
.contact__right-form-title {
  display: flex;

  width: 50%;
  height: 50px;
  flex-shrink: 0;
  margin-right: 5px;
}
.contact__right-form-title-title {
  margin-top: 12px;
}
/* .contact__left-form {
    
} */
.contact__right-form {
  margin: 40px;
  border-radius: 5%;
  background-color: beige;
  padding: 20px;
}
.contact__right-form button {
  background: #f7941e;
  border-radius: 6px;
  font-weight: 500;
  font-size: 18px;
  line-height: 24px;
  display: flex;
  justify-content: center;
  align-items: center;
  text-align: center;
  color: #fff;
  width: 100%;
  margin-top: 20px;
  height: 50px;
  transition: 0.3s;
  border: none;
}
/* Add global styles here */
</style>
